...
namespace Ikriv.Xml
{
    ...
    public class OverrideXml
    {
        ...
        {
            Commit();
            if (_currentType is null) throw new InvalidOperationException("Current type is not defined. Use Override<T>() to define current type");

            // attempt to verify that such member indeed exists
            const BindingFlags flags = BindingFlags.Instance | BindingFlags.Public;
            if (_currentType.GetProperty(name, flags) is null && _currentType.GetField(name, flags) is null)
            {
        ...
        {
            if (_attributes is not null && _currentType is not null)
            {
